...
export const FloatingLines: FC<FloatingLinesProps> = ({
    linesGradient,
    ...
    const targetMouseRef = useRef<Vector2>(
        new Vector2(-1000, -1000),
    );
    const currentMouseRef = useRef<Vector2>(
        new Vector2(-1000, -1000),
    );
    const targetInfluenceRef = useRef<number>(0);
    const currentInfluenceRef = useRef<number>(0);
    const targetParallaxRef = useRef<Vector2>(
        new Vector2(0, 0),
    );
    const currentParallaxRef = useRef<Vector2>(
        new Vector2(0, 0),
    );
    ...
    const topLineDistance = enabledWaves.includes('top')
        ...
        : 0.01;
    const middleLineDistance = enabledWaves.includes(
        'middle',
    )
        ? getLineDistance('middle') * 0.01
        : 0.01;
    const bottomLineDistance = enabledWaves.includes(
        'bottom',
    )
    ...
    useEffect(() => {
        ...
        if (linesGradient && linesGradient.length > 0) {
            const stops = linesGradient.slice(
                0,
                MAX_GRADIENT_STOPS,
            );
    ...
    return (
        <div
            ref={containerRef}
            className='size-full relative overflow-hidden floating-lines-container -z-10'
            style={{
